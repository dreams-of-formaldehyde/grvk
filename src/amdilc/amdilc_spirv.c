...
void ilcSpvUnwrapBuffer(
    ...
    unsigned wordCount)
{
    const IlcSpvBuffer srcBuffer = (IlcSpvBuffer) {
        .wordCount = wordCount,
        .wordSize = wordCount * sizeof(IlcSpvWord),
        .words = (IlcSpvWord*)src,
    };
...
unsigned getBufferIndex(SpvOp opCode) {
    ...
    } else if (opCode == SpvOpTypeStruct || opCode == SpvOpTypeArray || opCode == SpvOpTypePointer || opCode == SpvOpTypeRuntimeArray) {
